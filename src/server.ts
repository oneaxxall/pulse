...
export class Server {
     ...
     */
    public manager: AppManagerInterface;
    ...
    async start(callback?: CallableFunction) {
        ...
        try {
            ...
            if (this.options.debug) {
                Log.info('🥰 Menginisiasi pds-pusher-server....');
                Log.info('😇 Menginisiasi HTTP & WebSocket server...');
            }
            ...
            if (this.options.debug) {
                Log.info('🙂 Menginisiasi WebSocket listener dan channel...');
            }

            await this.configureWebsockets(server);

            if (this.options.debug) {
                Log.info('😚 Menginisiasi HTTP webserver...');
            }

            await this.configureHttp(server);
            await this.configureMetricsServer(metricsServer);
            ...
            await new Promise<void>((resolve, reject) => {
                server.listen(this.options.host, this.options.port, serverProcess => {
                    ...
                    if (!serverProcess) {
                        Log.errorTitle(`❌ GAGAL menjalankan server di ${this.options.host}:${this.options.port}`);
                        Log.error('Pastikan port tersebut tidak sedang digunakan oleh proses lain (seperti PM2 atau Supervisor).');
                        process.exit(1);
                        return reject();
                    }

                    Log.successTitle('✅ Server pusher berhasil buat');
                    Log.successTitle(`✅ Server WebSocket tersedia di 127.0.0.1:${this.options.port}`);
                    Log.successTitle(`💖 HTTP API Server tersedia di http://127.0.0.1:${this.options.port}`);

                    Log.successTitle(`📊 Endpoint /usage tersedia pada port ${this.options.metrics.port}.`);
                    Log.successTitle(`👥 Endpoint /users (Monitoring Koneksi) tersedia pada port ${this.options.metrics.port}.`);

                    if (this.options.metrics.enabled) {
                        Log.successTitle(`✅ Endpoint Prometheus /metrics tersedia pada port ${this.options.metrics.port}.`);
                    }
    ...
    stop(): Promise<void> {
        ...
        Log.br();
        Log.warning('❎ User tidak bisa connect ke pusher instance ini lagi...');
        Log.warning('💔 Server di tutup dan semua koneksi signaling akan di terminate.');
        Log.br();

        return this.wsHandler.closeAllLocalSockets().then(() => {
            return new Promise(resolve => {
                if (this.options.debug) {
                    Log.warningTitle('⚡ Semua socket sudah berhasil ditutup.');
                }
    ...
    setOptions(options: { [key: string]: any; }): void {
        for (let optionKey in options) {
            ...
            if (optionKey.match("^manager.array.apps.\\d+.id")) {
     ...
     * Set the app manager.
     */
    setAppManager(instance: AppManagerInterface): Promise<void> {
        return new Promise(resolve => {
            instance.init().then(() => {
            ...
            if (this.canProcessRequests()) {
                server = server.ws(this.url('/app/:id'), {
                    ...
                    upgrade: (res: HttpResponse, req: HttpRequest, context) => {
                        if (this.options.debug) {
                            Log.info(`📡 [WebSocket] Ada yang mengetuk! URL: ${req.getUrl()}, Parameter ID: ${req.getParameter(0)}`);
                        }
    ...
    protected configureMetricsServer(metricsServer: TemplatedApp): Promise<TemplatedApp> {
        return new Promise(resolve => {
            Log.info('🌟 Mendapatkan endpoints...');
            Log.br();

            metricsServer.get('/', (res, req) => this.httpHandler.healthCheck(res));
            metricsServer.get('/ready', (res, req) => this.httpHandler.ready(res));
            metricsServer.get('/usage', (res, req) => this.httpHandler.usage(res));
            metricsServer.get('/users', (res, req) => this.httpHandler.users(res));

            if (this.options.metrics.enabled) {
                metricsServer.get('/metrics', (res, req) => {
                    res.query = queryString.parse(req.getQuery());

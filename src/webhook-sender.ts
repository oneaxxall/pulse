            ...
            server.manager.findByKey(appKey).then(app => {
                ...
                async.each(app.webhooks, (webhook: WebhookInterface, resolveWebhook) => {
                    ...
                    const headers = {
                        ...
                        'User-Agent': `PdsPusherWebhooksAxiosClient/1.0 (Process: ${this.server.options.instance.process_id})`,
                    ...
                    if (webhook.url) {
                        ...
                        axios.post(webhook.url, payload, { headers }).then((res) => {
                            ...
                            this.webhookLogger.write(
                                this.webhookLogger.buildSuccessEntry(
                                    appKey,
                                    webhook.url,
                                    filteredPayloadEvents.map(e => e.name),
                            ...
                            this.webhookLogger.write(
                                this.webhookLogger.buildFailedEntry(
                                    appKey,
                                    webhook.url,
                                    filteredPayloadEvents.map(e => e.name),
                                    JSON.stringify(payload).length,

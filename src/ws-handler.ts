...
import { PresenceChannelManager, PresenceMember, PresenceMemberInfo } from './channels/presence-channel-manager';
...
export class WsHandler {
    ...
    onOpen(ws: WebSocket): any {
        ...
        if (this.server.options.debug) {
            Log.info(`🔌 Koneksi WebSocket baru. App Key: ${ws.appKey}`);
        }
    ...
    handleUpgrade(res: HttpResponse, req: HttpRequest, context): any {
        const rawAppKey = req.getParameter(0);
        if (this.server.options.debug) {
            Log.info(`🔍 [Upgrade] Raw App Key dari URL: "${rawAppKey}"`);
        }
    ...
    subscribeToChannel(ws: WebSocket, message: PusherMessage): any {
        ...
        if (channel.length > Number(ws.app.maxChannelNameLength)) {
            let broadcastMessage = {
                ...
                if (channelManager instanceof PresenceChannelManager && ws.presence.has(channel)) {
                    ...
                    this.server.adapter.getChannelMembers(ws.app.id, channel, false).then(members => {
                        if (!members.has(member.user_id as string)) {
                            this.server.webhookSender.sendMemberRemoved(ws.app, channel, member.user_id as string);
    ...
    handleClientEvent(ws: WebSocket, message: PusherMessage): any {
        ...
        if (event.length > Number(ws.app.maxEventNameLength)) {
            let broadcastMessage = {
                event: 'pusher:error',
                channel,
                data: {
        ...
        if (payloadSizeInKb > Number(ws.app.maxEventPayloadInKb)) {
            let broadcastMessage = {
                event: 'pusher:error',
                channel,
                data: {
    ...
    protected checkForValidApp(ws: WebSocket): Promise<App|null> {
        return this.server.manager.findByKey(ws.appKey);
    }
    ...
    protected checkAppConnectionLimit(ws: WebSocket): Promise<boolean> {
        return this.server.adapter.getSocketsCount(ws.app.id).then(wsCount => {
            let maxConnections = Number(ws.app.maxConnections) || -1;

...
export abstract class SqlAppManager extends BaseAppManager {
    ...
    init(): Promise<void> {
        return new Promise((resolve, reject) => {
            this.connection.raw('SELECT 1').then(() => {
                Log.successTitle(`✅ Berhasil terhubung ke database (${this.knexClientName()})`);
                resolve();
            }).catch(err => {
                Log.error(`❌ Gagal terhubung ke database (${this.knexClientName()}): ${err.message}`);
                reject(err);
            });
        });
    }
    ...
    protected selectById(id: string): Promise<App[]> {
        if (this.server.options.debug) {
            Log.info(`🔍 Mencari App berdasarkan ID: ${id} di tabel: ${this.appsTableName()}`);
        }
        return this.connection<App>(this.appsTableName())
            ...
            .then(apps => {
                if (this.server.options.debug) {
                    Log.info(`📊 Hasil ID ${id}: ${apps.length} ditemukan.`);
                }
                return apps;
            });
    }
    ...
    protected selectByKey(key: string): Promise<App[]> {
        if (this.server.options.debug) {
            Log.info(`🔍 Mencari App berdasarkan KEY: ${key} di tabel: ${this.appsTableName()}`);
        }
        return this.connection<App>(this.appsTableName())
            .where('key', key)
            .select('*')
            .then(apps => {
                if (this.server.options.debug) {
                    Log.info(`📊 Hasil KEY ${key}: ${apps.length} ditemukan.`);
                }
                return apps;
            });
    }

...
export class HttpHandler {
    ...
    acceptTraffic(res: HttpResponse) {
        this.attachMiddleware(res, [
            ...
            if (Number(threshold) < percentUsage) {
    ...
    users(res: HttpResponse) {
        console.log('🔍 Request masuk ke endpoint /users...');

        res.cork(() => {
            const namespaces = this.server.adapter.getNamespaces();
            const allUsers: any[] = [];

            namespaces.forEach((namespace, appId) => {
                namespace.sockets.forEach((ws, socketId) => {
                    allUsers.push({
                        id: socketId,
                        app_id: appId,
                        ip: ws.ip || 'unknown',
            ...
            res.end(JSON.stringify({
                total_connections: allUsers.length,
                users: allUsers,
            }));
        });
    }
    ...
    protected checkMessageToBroadcast(message: PusherApiMessage, app: App): Promise<PusherApiMessage> {
        return new Promise((resolve, reject) => {
            ...
            if (payloadSizeInKb > Number(app.maxEventPayloadInKb)) {
                return reject({
    ...
    protected appMiddleware(res: HttpResponse, next: CallableFunction): any {
        return this.server.manager.findById(res.params.appId).then(validApp => {
            if (!validApp) {
     ...
     * Sended from API query auth_signature
     */

...
However, when queueing using Redis, Socket will queue the job details in Redis and the job will eventually be processed later by one of Socket's queue workers so that your main server response time does not suffer.

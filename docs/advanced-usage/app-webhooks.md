...
Starting with Socket `0.23`, you can filter those channels you want to receive based on their name.
...
Starting with Socket `0.24`, [thanks to @stayallive's PR](https://github.com/Socket/Socket/pull/226), you can pass additional headers that will be sent within the webhook request:
...
If you have a lot of webhooks going on, consider increasing this value according to your needs. In the submitted [Pull Request](https://github.com/Socket/Socket/pull/249), a use case was the AWS Lambda functions webhook handling, which incurs costs based on how much the function is running and based on how many requests are done. For high-traffic apps that use those webhooks, increasing the batch duration is important, so that requests are sent at a lower rate, but with more events.

```bash
# Setting the duration to 1000ms = 1s
WEBHOOKS_BATCHING=1 WEBHOOKS_BATCHING_DURATION=1000 Socket start
```
